...
### Data sourcing (today) and roadmap

- **Today — scraped catalog file:** Structured rows (parts, compat, install/repair text, etc.) are stored in **`catalog.json`**, built and updated through the **rate-limited scrape/merge** script against PartSelect PDP HTML (`scrape-parts.mjs`), with optional **`fetch_part_page`** as a slow, read-only gap-fill. This keeps the submission **self-contained and diffable** in Git; it is not pretending to be a live production catalog service.
- **Today — simple RAG:** **`semantic_search`** is intentionally lightweight: **`embeddings.json`** holds chunk text + vectors (from `generate-embeddings.mjs`); at query time we do **in-memory cosine** over that file—**no hosted vector database**. It is enough for demo-scale experiential retrieval (stories, FAQ, install snippets) and easy to reason about; it is not tuned for large corpora or high QPS.
- **Later — real data in a database:** For production, **`catalog.json` would be replaced by a database** (or upstream PIM / search API) as the system of record, while **`retrieveExact`** remains the single retrieval seam so **cards, citations, and tests** keep the same contract. RAG would graduate to **batch indexing + ANN** (or a managed vector store) when content volume and traffic require it.
...
**20 golden cases** (17 single-turn + 3 multi-turn) in `web/scripts/goldenCases.mjs` cover the six query categories, clarify shapes, OOS, and multi-turn context. With the LLM enabled, assertions on `tool_trace` check that the expected tool was chosen for representative turns.
